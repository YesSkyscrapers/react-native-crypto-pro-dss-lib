#!/bin/bash
# $1 - text_vmaddr, $2 - text_offset, $3 - text_size, $4 - var_offset, $5 - program name > hack()
hack()
{
  global_var_offset=$4
  global_var_offset=$(($global_var_offset + ${offsets_arr[$i]}))
  main_off=$[$main_addr-$1]
  main_off=$(($main_off + ${offsets_arr[$i]}))
  size=$[$3-$main_addr+$1]
  echo "main_addr:$main_addr main_off:$main_off size:$size"
  dd if="$5" ibs=$main_off skip=1 of=${TMP_FILE}.1
  dd if=${TMP_FILE}.1 ibs=$size count=1 of=$TMP_FILE
  checksum=`"$CPVERIFY" -mk "$TMP_FILE" -alg GR3411_2012_256`
...
while [ $i -lt $archs_number ]; do
  ...
  main_addr=`nm -arch $arch "$1"|grep -e "[[:space:]]_main$"|awk '{print toupper($1)}'`
  main_addr=`echo "ibase=16; $main_addr" |bc`
  case $3 in
    no_xcode) main_addr=$[$main_addr-1] ;;
    *) ;;
  esac

hack  `otool -l -arch $arch "$1"|awk -v var_vmaddr=$var_offset '
